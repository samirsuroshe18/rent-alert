...
const uploadOnCloudinary = async (localFilePath) => {
    try {
        ...
        if (fs.existsSync(localFilePath)) {
            response = await cloud.uploader.upload(localFilePath, { resource_type: "auto", folder: "rent-alert", });
...
const deleteCloudinary = async (cloudinaryFilePath) => {
    try {
        ...
        cloud.uploader.destroy("rent-alert/"+ publicId).then((result) => console.log(result));
